...
class BrowserWorldModel(WorldModel[BrowserState, BrowserAction, str]):
    ...
    async def execute_browser_action(
        ...
        try:
            new_base64_img = await get_screenshot()
        except Exception as e:
            print(f"{RED}[DEBUG] Error getting screenshot after action: {e}{RESET}")
            new_base64_img = "Error: Unable to retrieve screenshot"

        # try:
        #     new_web_ele = await get_web_element_rect()
        # except Exception as e:
        #     print(f"{RED}[DEBUG] Error getting URL after action: {e}{RESET}")
        #     new_web_ele = "Error: Unable to retrieve web elements"
        print(f"{GREEN}[DEBUG] After action execution - New URL: {new_url}{RESET}")
        return new_base64_img, new_dom, new_url
...
if __name__ == "__main__":
    ...
    try:
        ...
        asyncio.run(
            main(
                objective="Find a pet-friendly hotel with parking available in downtown Toronto for the stay of February 24-26, 2024 on web https://www.booking.com/",
                eval_mode=False,
            )
        )
    finally:
