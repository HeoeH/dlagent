            ...
            for attempt in range(retries):
                try:
                    ...
                    if attempt < retries - 1:
                        print(f"[DEBUG] Action failed with error: {e}. Retrying...")
                        await asyncio.sleep(delay)
                    else:
                        print(f"[DEBUG] Action failed with error: {e}. No more retries.")
                        return False
            ...
            for attempt in range(max_retries):
                try:
                    if action.type == ActionType.GOTO_URL:
                        ...
                        print("Action - ENTER_TEXT_AND_CLICK")
                        ...
                        print(f"Action - SCROLL_{direction.upper()}")
                        ...
                        result = f"Pressed keys: {mapped_keys}"
                        print("Action - KEY_PRESS")
                    elif action.type == ActionType.NEW_TAB:
                        browser_ctx = await self.playwright_manager.get_browser_context()
                        page = await browser_ctx.new_page()
                        await page.goto("https://www.google.com")
                        result = "Opened new tab"
                        print("Action - NEW_TAB")
                    elif action.type == ActionType.GO_BACK:
                        await page.go_back()
                        result = "Navigated back"
                        print("Action - GO_BACK")
