...
async def get_dom_with_content_type(
    ...
    # )
    extracted_data= await format_elements(extracted_data)
    return extracted_data  # type: ignore

async def format_elements(data):
    def traverse_elements(elements):
        format_ele_text = []
        
        for element in elements:
            mmid = element.get('mmid')
            if mmid:  # 只处理有mmid的元素
                label_text = element.get('name', '')
                ele_aria_label = element.get('text', '')
                ele_tag_name = element.get('tag', '')
                
                # 格式化输出
                if ele_aria_label and (ele_aria_label != label_text):
                    format_ele_text.append(f"[{mmid}]: <{ele_tag_name}> \"{label_text}\", \"{ele_aria_label}\";")
                else:
                    format_ele_text.append(f"[{mmid}]: <{ele_tag_name}> \"{label_text}\";")
            
            # 如果当前元素还有子元素，则递归处理
            children = element.get('children', [])
            if children:
                format_ele_text.extend(traverse_elements(children))
        
        return format_ele_text
    
    formatted_output = traverse_elements(data.get('children', []))
...
#     #     user_success_message, message_type=MessageType.ACTION
#     # )
#     extracted_data= format_extracted_data(extracted_data)
#     return extracted_data  # type: ignore
...
#             if mmid:  # 只处理有mmid的元素
#                 label_text = element.get('name', '')
#                 ele_aria_label = element.get('aria-label', '')
#                 ele_tag_name = element.get('tag', '')
#                 ele_role = element.get('role', '')
#                 ele_tag_type = element.get('tag_type', '')
                
#                 # 格式化输出
#                 if ele_aria_label and (ele_aria_label != label_text):
#                     format_ele_text.append(f"[{mmid}]: <{ele_tag_name}> \"{label_text}\", \"{ele_aria_label}\", \"{ele_role}\";")
#                 else:
#                     format_ele_text.append(f"[{mmid}]: <{ele_tag_name}> \"{label_text}\", \"{ele_role}\";")
